...
class NacosChannel(_Channel):

    def __init__(self, service_name, nacos_client, interceptor):
        self.service_name = service_name
        self.nacos_client = nacos_client
        self._channel = self.get_channel()
        super(NacosChannel, self).__init__(self._channel, interceptor)
        # TODO subscribe and unsubscribe

    def get_instances(self):
      return self.nacos_client.list_naming_instance(self.service_name)
    ...
    def unary_unary(self,
                    ...
                    request_serializer=None,
                    response_deserializer=None):
        thunk = lambda m: self.get_channel().unary_unary(m, request_serializer,
                                                    response_deserializer)
        if isinstance(self._interceptor, grpc.UnaryUnaryClientInterceptor):
            return _UnaryUnaryMultiCallable(thunk, method, self._interceptor)
        else:
    ...
    def unary_stream(self,
                     ...
                     request_serializer=None,
                     response_deserializer=None):
        thunk = lambda m: self.get_channel().unary_stream(m, request_serializer,
                                                     response_deserializer)
        if isinstance(self._interceptor, grpc.UnaryStreamClientInterceptor):
            return _UnaryStreamMultiCallable(thunk, method, self._interceptor)
        else:
    ...
    def stream_unary(self,
                     ...
                     request_serializer=None,
                     response_deserializer=None):
        thunk = lambda m: self.get_channel().stream_unary(m, request_serializer,
                                                     response_deserializer)
        if isinstance(self._interceptor, grpc.StreamUnaryClientInterceptor):
            return _StreamUnaryMultiCallable(thunk, method, self._interceptor)
        else:
    ...
    def stream_stream(self,
                      ...
                      request_serializer=None,
                      response_deserializer=None):
        thunk = lambda m: self.get_channel().stream_stream(m, request_serializer,
                                                      response_deserializer)
        if isinstance(self._interceptor, grpc.StreamStreamClientInterceptor):
            return _StreamStreamMultiCallable(thunk, method, self._interceptor)
        else:
...
class RoundrobinChannel(NacosChannel):
    ...
    def __init__(self, service_name, nacos_client, interceptor):
        self._cur_index = 0
        super(RoundrobinChannel, self).__init__(service_name, nacos_client, interceptor)
