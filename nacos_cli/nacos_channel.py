...
class _UnaryUnaryMultiCallable(grpc.UnaryUnaryMultiCallable):
    ...
    def _blocking(self, request, timeout, metadata, credentials, wait_for_ready,
        ...
        if state is None:
            ...
            call = self._channel.get_channel().segregated_call(
...
class NacosChannel(grpc.Channel):
    """A cygrpc.Channel-backed implementation of grpc.Channel."""

    def __init__(self, service_name, nacos_client, options, credentials, compression):
        """Constructor.
        ...
        #     _common.encode(self.select_target()), _augment_options(core_options, compression),
        #     credentials)
    ...
    def get_instances(self):
      return self.nacos_client.list_naming_instance(self.service_name, healthy_only=True)

    def select_target(self):
        # pick first
        s = self.get_instances()['hosts'][0]
        return s['ip'] + ':' + str(s['port'])

    def get_channel(self):
        target = self.select_target()
        print(f'use channel {target}')
        try:
            return self._channels[target]
        except KeyError:
            channel = cygrpc.Channel(
            _common.encode(self.select_target()), _augment_options(self.core_options, self.compression),
            self.credentials)
            self._channels[target] = channel
            self._call_state = _ChannelCallState(channel)
    ...
    def unary_unary(self,
        ...
        return _UnaryUnaryMultiCallable(
            self, _channel_managed_call_management(self._call_state),
    ...
    def _close(self):
        ...
        for target in self._channels:
            channel = self._channels[target]
            channel.close(cygrpc.StatusCode.cancelled, 'Channel closed!')
            cygrpc.fork_unregister_channel(channel)

    def _close_on_fork(self):
        self._unsubscribe_all()
        for target in self._channels:
            channel = self._channels[target]
            channel.close_on_fork(cygrpc.StatusCode.cancelled,
...
class RandomChannel(NacosChannel):
    """Random gRPC load balance channel."""

    def select_target(self):
        instances = self.get_instances()['hosts']
        addr_num = len(instances)
        if addr_num == 0:
          raise ValueError('No channel.')
        addr_idx = random.randint(0, addr_num - 1)
        addr = instances[addr_idx]
        return addr['ip'] + ':' + str(addr['port'])


class RoundrobinChannel(NacosChannel):
    """Roundrobin gRPC load balance channel."""

    def __init__(self, service_name, nacos_client, options, credentials, compression):
        self._cur_index = 1
        super(RoundrobinChannel, self).__init__(service_name, nacos_client, options, credentials, compression)
    ...
    def select_target(self):
        ...
        addr = instances[self._cur_index % addr_num]
